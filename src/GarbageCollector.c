...
extern end, etext, edata; /* Provided by the linker. Text-segment, initialize/unitialized data segment address. */
    ...
    for (; sp < end; sp++) {
        ...
        while (current_ptr) {
            ...
            if ((uintptr_t)(current_ptr + 1) <= value_pointed_at && value_pointed_at < (uintptr_t)(current_ptr + 1) + current_ptr->size) {
                printf("    The heap address \x1b[34m%p\033[0;37m is reachable and marked.\n", (void*) value_pointed_at);
